...
;----------------------------------------------------------
        .DATA
N       DD      ?          ; number of primes to print
COUNT   DD      0
NUM     DD      2
FLAG    DW      1
...
START:  ;clear screen
        ...
        MOV     N, CX
        ;overflow check
        CMP     CX, 6543    ;6542th prime number doesn't fit in 16bits regiter
        JG      OVERFLOW
        ;display line break
        MOV     DX, 13
        MOV     AH, 2
...
OVERFLOW:
        MOV AX,0600H        ;06 TO SCROLL & 00 FOR FULLJ SCREEN
        MOV BH,74H          ;ATTRIBUTE 7 FOR BACKGROUND AND 1 FOR FOREGROUND
        MOV CX,0000H        ;STARTING COORDINATES
        MOV DX,014FH        ;ENDING COORDINATES
        INT 10H
        MOV     AH, 9
        MOV     DX, OFFSET MSG2
        INT     21h
        MOV     AH, 0       ;wait for any key to continue
        INT     16h
        JMP     START
MAIN    ENDP
;----------------------------------------------------------
